...
import csv
import argparse
from city import fetch_all_cities
...
class scraper:
    def __init__(self,city,query):
        ...
        self.out = open(city + query + '.csv', 'w')
        self.fields = ['Name', 'Address', 'URL', 'Latitude', 'Longitude']
        self.writer = csv.DictWriter(self.out, delimiter=',', fieldnames=self.fields)
        self.writer.writerow({x: x for x in self.fields})
    def scrap(self):
    ...
    def extract_business_details(self,data):
        bsns = business(data,self.city,self.query, self.writer)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--brand', '-b', help='name of the brand to search', required=True)
    parser.add_argument('--city', '-c', help='name of the city')
